/* William Anderson - 4/19/2021 - C Homework */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
item_t *create_item(int id, char description[], double price, int
  ...
  if (newItemStructure != NULL) {
    newItemStructure->id = id;
    newItemStructure->price = price;
    newItemStructure->quantity_available = quantity_available;
    newItemStructure->description = descString;
    ...
    return NULL; /* Won't ever be executed but makes the compiler happy :) */
  }
}
...
double compute_price_of_order(ordered_item_t *order[], int order_length) {
  ...
  for (i = 0; i < order_length; i++) {
    runningTotal += order[i]->item->price * (double) order[i]->quantity_ordered;
  }
...
void print_order(ordered_item_t *order[], int order_length) {
  ...
  if (order == NULL || order_length <= 0) {
    ...
    printf("Total price of the order: $0.00\n");
    return;
  }
...
void add_item_to_order(ordered_item_t *(*new_order[]), int *new_order_length,
  ...
  int i = 0;
  ordered_item_t *newItem;

  if (old_order == NULL || old_order_length <= 0) {
    old_order_length = 0;
  }

  for (i = 0; i < old_order_length; i++) {
    if (old_order[i]->item->id == item->id) {
      *new_order = old_order;
  ...
  *new_order = (ordered_item_t **) calloc(*new_order_length, sizeof(ordered_item_t *));
  ...
  newItem->item = item;
  newItem->quantity_ordered = quantity;
...
int delete_item_from_order(ordered_item_t *(*new_order[]), int *new_order_length,
  ...
  if (item_index < 0 || item_index >= old_order_length) {
    return 0;
  }
